...
struct dtCrowdAgentParams
{
	...
	float maxAcceleration;				///< Maximum allowed m_acceleration. [Limit: >= 0]
...
struct dtCrowdAgent
{
	...
	float dvel[3];		///< The desired m_velocity of the agent. Based on the current path, calculated from scratch each frame. [(x, y, z)]
	float nvel[3];		///< The desired m_velocity adjusted by obstacle avoidance, calculated from scratch each frame. [(x, y, z)]
	float vel[3];		///< The actual m_velocity of the agent. The change from nvel -> vel is constrained by max m_acceleration. [(x, y, z)]
	...
	float targetPos[3];					///< Target position of the movement request (or m_velocity in case of DT_CROWDAGENT_TARGET_VELOCITY).
...
public:
	...
	/// Submits a new move request for the specified agent.
	///  @param[in]		idx		The agent index. [Limits: 0 <= value < #getAgentCount()]
	///  @param[in]		vel		The movement m_velocity. [(x, y, z)]
	/// @return True if the request was successfully submitted.
	...
	/// Gets the m_velocity sample count.
	/// @return The m_velocity sample count.
...
like agent m_velocity and steering to avoid other agents, that is up to you to 
implement. Unless, of course, you decide to use #dtCrowd.

Basically, you add an agent to the crowd, providing various configuration 
settings such as maximum speed and m_acceleration. You also provide a local 
target to more toward. The crowd manager then provides, with every update, the 
new agent position and m_velocity for the frame. The movement will be 
...
and m_acceleration. But in order for the crowd manager to do its thing, it can't 
allow you to constantly be giving it overrides to position and m_velocity. So 
