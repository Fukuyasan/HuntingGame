...
//			ag->corridor.trimInvalidPath(agentRef, agentPos, m_navquery, &m_filter);
		...
		// If the agent does not have move target or is controlled by m_velocity, no need to recover the target nor replan.
		if (ag->targetState == DT_CROWDAGENT_TARGET_NONE || ag->targetState == DT_CROWDAGENT_TARGET_VELOCITY)
			continue;
...
void dtCrowd::update(const float dt, dtCrowdAgentDebugInfo* debug)
{
	...
	for (int i = 0; i < nagents; ++i)
	{
		...
		if (ag->params.updateFlags & DT_CROWD_SEPARATION)
		{
			...
			if (w > 0.0001f)
			{
				// Adjust desired m_velocity.
				dtVmad(dvel, dvel, disp, 1.0f/w);
				// Clamp desired m_velocity to desired speed.
	...
	for (int i = 0; i < nagents; ++i)
	{
		...
		{
			// If not using m_velocity planning, new m_velocity is directly the desired m_velocity.
