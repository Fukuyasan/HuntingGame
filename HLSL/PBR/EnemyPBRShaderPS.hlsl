...
#define CLACK_RED    (1 << 0)
#define CLACK_GREEN  (1 << 1)
#define CLACK_BLUE   (1 << 2)
#define CLACK_YELLOW (1 << 3)
...
Texture2D decalTex			  : register(t22);

SamplerState samplerState[5]  : register(s0);

cbuffer ClackBuffer : register(b5)
{
	int clack;
};
...
float4 main(VS_OUT pin) : SV_TARGET
{
	...
	// 部位破壊
	float4 decal_color = decalTex.Sample(samplerState[ANISOTROPIC], pin.texcoord);
	
	// 部位破壊テクスチャの色によって破壊された場所を制御する
	if (clack & CLACK_RED)
		N = lerp(N, decal_color.r, decal_color.r);
	if (clack & CLACK_GREEN)
		N = lerp(N, decal_color.g, decal_color.g);
	if (clack & CLACK_BLUE)
		N = lerp(N, decal_color.b, decal_color.b);
	if (clack & CLACK_YELLOW)
		N = lerp(N, decal_color.r * decal_color.g, decal_color.r * decal_color.g);
