...
Texture::Texture(const char* filename)
{
	...
	LoadTexture(filename, device, this->shaderResourceView.GetAddressOf(), &this->texture2dDesc);
}

inline HRESULT Texture::LoadTexture(const char* filename, ID3D11Device* device, ID3D11ShaderResourceView** pSrv, D3D11_TEXTURE2D_DESC* textureDesc)
{
	// パスを分解
	...
	HRESULT hr = DirectX::CreateWICTextureFromFile(device, wfilepath, resource.GetAddressOf(), pSrv);
	// DDSとして読み込んでみる
	if (FAILED(hr))
	{
		hr = DirectX::CreateDDSTextureFromFile(device, wfilepath, resource.GetAddressOf(), pSrv);
	}
	...
	if (FAILED(hr))
	{
		...
		if (pixels != nullptr)
		{
			...
			hr = device->CreateShaderResourceView(texture2d.Get(), nullptr, pSrv);
			_ASSERT_EXPR(SUCCEEDED(hr), HRTrace(hr));
