...
void StageMain::Start()
{
	...
	// モデル行列更新
	object->GetModel()->UpdateTransform(object->transform.m_transform);


	// Jsonからステージ読み込み
	//LoadStageObjects("Data/Json/StageData.json");
...
void StageMain::LoadStageObjects(const char* filename)
{
	...
	{
		// " "から前を区切る
		std::string stage = sprit(stageData.stageParam.at(i).name, " ");
		...
		// ステージ読み込み
		models.at(i) = std::make_unique<Model>(fileName);
