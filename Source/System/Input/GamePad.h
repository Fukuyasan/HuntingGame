...
public:
	...
	// スロット設定
	void SetSlot(int s) { this->slot = s; }
	...
	// 左スティック入力状態の取得
	float GetAxisLStick() const { return sqrtf(axisLx * axisLx + axisLy * axisLy); }

	// 右スティック入力状態の取得
	float GetAxisRStick() const { return sqrtf(axisRx * axisRx + axisRy * axisRy); }
